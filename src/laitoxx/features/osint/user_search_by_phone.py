...
def search_by_number():
    ...
    if not validate_phone(phone_for_validation):
        print(
            f"{Color.DARK_GRAY}[{Color.DARK_RED}⛧{Color.DARK_GRAY}]{Color.DARK_RED} Invalid number. Try again (digits, with or without '+')."
        )
        return
    ...
    try:
        ...
        number_type = (
            "Mobile" if phonenumbers.number_type(parsed_number) == 1 else "Fixed line or other"
        )
    ...
    try:
        ...
        if response.status_code == 200 and "User not found" not in response.text:
            print(
                f"{Color.DARK_GRAY}[{Color.DARK_RED}⛧{Color.DARK_GRAY}]{Color.LIGHT_GREEN} VK: response received (verify manually — may require login to confirm)."
            )
        else:
            print(
    ...
    print(
        f"{Color.DARK_GRAY}[{Color.DARK_RED}⛧{Color.DARK_GRAY}]{Color.LIGHT_BLUE} Avito: manual check required → {avito_url}"
    )
