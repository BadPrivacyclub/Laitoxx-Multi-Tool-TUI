...
def check_login(
    ...
    if soup:
        ...
        if masked_phone:
            masked_phone_text = (
                masked_phone.find("div", {"class": "ext-registration_stub_small_header"})
            ).get_text()

        if masked_email:
            masked_email_text = (
                masked_email.find("div", {"class": "ext-registration_stub_small_header"})
            ).get_text()
...
def phone_number_lookup(number: str, log: Callable[[str], None] = print) -> None:
    api_url = f"https://phone-number-api.com/json/?number={number}"
    try:
        response = requests.get(api_url, timeout=REQUEST_TIMEOUT)
        response.raise_for_status()
        phone_rsp = json.loads(response.text)
    except requests.RequestException as exc:
        log(f"[!] Phone lookup request error: {exc}")
        return
    except json.JSONDecodeError:
        log("[!] Failed to parse phone-number-api.com response")
        return
...
def savedata_phone_lookup(phone: str, log: Callable[[str], None] = print) -> None:
    ...
    number = num[:-3]
    if not number:
        log("Not enough digits for intelx query.")
        return
    ...
    try:
        ...
        log(f"[!] Intelx phone request error: {exc}")
        return

    if response.status_code != 200:
        log("Nothing found (intelx phone).")
        return
    ...
    match_idx = 0
    for row in reader:
        if any(phone.lower() in str(v).lower() for v in row.values()):
            if not matched:
                log("Found rows (intelx phone):")
                matched = True
            match_idx += 1
            ...
            _log_separator(log)
    if not matched:
        log("No matches found (intelx phone).")
...
def savedata_email_lookup(email: str, log: Callable[[str], None] = print) -> None:
    ...
    try:
        ...
        log(f"[!] Intelx email request error: {exc}")
        return

    if response.status_code != 200:
        log("Nothing found (intelx email).")
        return
    ...
    match_idx = 0
    for row in reader:
        if any(email.lower() in str(v).lower() for v in row.values()):
            if not matched:
                log("Found rows (intelx email):")
                matched = True
            match_idx += 1
            log(f"Record #{match_idx}")
            for key, value in row.items():
            ...
            _log_separator(log)
    if not matched:
        log("No matches found (intelx email).")
...
def telegram_profile(username: str, log: Callable[[str], None] = print) -> None:
    ...
    if response.status_code != 200:
        log("Telegram account not found.")
        return
    ...
    if name_span:
        log(f"Name: {name_span.text}")
        google_search(info_name=name_span.text, log=log)
    if description_div:
        log(f"Description: {description_div.text}")
...
def _phone_search_flow(phone: str, log: Callable[[str], None]) -> None:
    ...
    if not clean_phone:
        log("Phone number not provided.")
        return

    _log_separator(log)
    log(f"Searching by phone: {clean_phone}")
    _log_separator(log)
...
def _telegram_search_flow(username: str, log: Callable[[str], None]) -> None:
    ...
    if not username:
        log("Username not provided.")
        return

    _log_separator(log)
    log(f"Searching Telegram: {username}")
    _log_separator(log)
...
def _interactive_prompt(log: Callable[[str], None]) -> tuple[str | None, str | None]:
    ...
    log(f"2) {Color.GREEN}Search by email{Color.RESET}")
    log(f"3) {Color.GREEN}Search by Telegram{Color.RESET}")

    choice = input(f"{Color.DARK_RED}Select option (1-3): {Color.RESET}").strip()
    ...
    prompt_map = {
        "phone": "Enter phone number (without +): ",
        "email": "Enter email: ",
...
def data_search_tool(input_data: dict[str, str] | None = None) -> None:
    ...
    if not mode or not value:
        log("Insufficient data for search.")
        return
    ...
    if mode == "phone":
        ...
        log(f"Unknown search type: {mode}")
