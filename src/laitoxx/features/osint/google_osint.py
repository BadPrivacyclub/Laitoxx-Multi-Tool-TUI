...
def _query_from_data(data):
    query = str(data.get("query", "")).strip()
    if query:
        return query

    parts = []
    base_query = str(data.get("base_query", "")).strip()
    if base_query:
        parts.append(base_query)

    for item in data.get("operators", []):
        if isinstance(item, dict):
            op = str(item.get("name", "")).strip()
        ...
        if not value:
            continue
        if op == "custom" or op in ("numrange", "daterange"):
            parts.append(value)
        else:
...
def google_osint(data=None):
    """
    Constructs advanced Google dorking queries and opens them in the browser.
    """
    if data:
        final_query = _query_from_data(data)
        if not final_query:
            print("No query components provided.")
            return

        selected_engines = data.get("engines") or ["google"]
        if "all" in selected_engines:
            selected_engines = ["google", "bing", "duckduckgo", "yandex"]

        print(f"Generated Dork:\n{final_query}\n")
        for engine in selected_engines:
            url = _search_url(engine, final_query)
            if not url:
                continue
    ...
    engines_map = {"1": "google", "2": "bing", "3": "duckduckgo", "4": "yandex", "5": "all"}
...
def show_dork_examples():
    ...
    examples = [
        ...
        ("filetype:pdf site:gov confidential", "Find confidential PDFs on government sites"),
        ("inurl:login.asp intitle:admin", "Find ASP login pages with admin in title"),
        ("site:example.com ext:sql intext:password", "Find SQL files with passwords on a site"),
        ...
        ("ext:(sql|db|bak|conf) intext:password", "Find exposed database files with passwords"),
        ('site:github.com "password" filetype:env', "Find exposed .env files on GitHub"),
        ('inanchor:"click here" site:example.com', "Find pages linking with 'click here'"),
        ...
        ('"data science" after:2022 before:2024', "Data science content from 2022-2024"),
        ...
        ('("login" OR "admin") NEAR/5 "password" site:example.com', "Login/admin near password"),
        (
            "(site:example.com OR site:example.org) filetype:pdf confidential",
