...
class UsernameChecker:
    ...
    def _check_server_redirects(self, facts: _Facts, username: str) -> tuple[str, _Verdict | None]:
        ...
        if facts["history"] or initial_norm != final_norm:
            if self._is_benign_redirect(initial_url, final_url, username):
                ...
                return final_norm, ("not_found", f"Redirected away from profile URL: {final_url}")
    ...
    def _check_baseline(
        ...
        if (
            facts["content_length"] < MIN_PROFILE_BODY_SIZE
            and facts["content_length"] <= baseline["length"] + 200
        ):
            return (
                "not_found",
