...
MAX_CHAIN_LENGTH = 2_000
MAX_NUM_CHAINS = 5_000
...
def reduce_function(hashed_text, charset, max_len, step=0):
    """Maps a hash back to a plaintext using a step-varied reduction function."""
...
def _validate_generation_options(
    charset, chain_length, num_chains, password_len, output_file, salt_length
):
    if not charset:
        raise ValueError("Charset is required.")
    if len(charset) > MAX_CHARSET_LENGTH:
        raise ValueError(f"Charset is too long; max {MAX_CHARSET_LENGTH} characters.")
    chain_length = _bounded_int(chain_length, 500, minimum=1, maximum=MAX_CHAIN_LENGTH)
    num_chains = _bounded_int(num_chains, 1000, minimum=1, maximum=MAX_NUM_CHAINS)
    max_chains_for_work = max(1, MAX_TOTAL_OPERATIONS // chain_length)
    num_chains = min(num_chains, max_chains_for_work)
    password_len = _bounded_int(password_len, 6, minimum=1, maximum=MAX_PASSWORD_LENGTH)
    salt_length = _bounded_int(salt_length, 8, minimum=1, maximum=MAX_SALT_LENGTH)
    output_file = _safe_output_filename(output_file)
    return chain_length, num_chains, password_len, output_file, salt_length
...
def generate_rainbow_table(
    ...
    try:
        chain_length, num_chains, password_len, output_file, salt_length = (
    ...
    if use_salt:
        salt_used = secrets.token_hex(salt_length // 2)[:salt_length]
    ...
    try:
        with open(output_file, "w", newline="") as f:
            ...
            for i in range(num_chains):
                ...
                writer.writerow([start_plaintext, current_plaintext])
...
def rainbow_table_tool(data=None):
    ...
    if data:
        charset = str(data.get("charset", "")).strip()
        algorithm = str(data.get("algorithm", "md5")).lower().strip()
        chain_length = data.get("chain_length", 500)
        num_chains = data.get("num_chains", 1000)
        ...
        print(
            "- Chain length: How many hash/reduce operations per chain (higher = better coverage but slower)"
        )
        print(
            "- Number of chains: How many starting points (higher = better coverage but larger file)"
        )
        print("- Password length: Maximum length of passwords to generate")
        print(
            "- Use salt: Whether to add salt to hashes (makes cracking harder but table specific)"
        )
        print("- Salt length: Length of salt in characters (only used if salt is enabled)")
        print()
        print(
            "Example: charset='abc123', algorithm='md5', chain_length=1000, chains=10000, length=4"
        )
        print("This would generate a table for 4-character passwords using a,b,c,1,2,3")
        print()

        charset = input(
            "Enter the character set (e.g., 'abcdefghijklmnopqrstuvwxyz0123456789'): "
        ).strip()
