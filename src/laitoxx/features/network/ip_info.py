...
def _layer_ports(ip: str):
    ...
    if tags:
        tag_color = (
            Color.RED if any(t in tags for t in ["malware", "compromised", "c2"]) else Color.YELLOW
        )
    ...
    if ABUSEIPDB_API_KEY:
        ...
        if r_ab:
            try:
                ...
                color = (
                    Color.RED if score > 50 else (Color.YELLOW if score > 0 else Color.LIGHT_GREEN)
                )
                _row(
                    "AbuseIPDB score", f"{score}/100  (reports: {total}, last: {last})", color=color
                )
    ...
    if VIRUSTOTAL_API_KEY:
        ...
        if r_vt:
            try:
                ...
                _row("VirusTotal", f"malicious={mal} suspicious={sus} harmless={harm}", color=color)
