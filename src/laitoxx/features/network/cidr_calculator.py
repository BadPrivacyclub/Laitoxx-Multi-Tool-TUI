...
def cidr_calculator_tool(data=None):
    ...
    if subnet_count and subnet_count > 1:
        ...
        if new_prefix > max_prefix:
            ...
            print(
                f"{Color.DARK_GRAY}  New prefix: {Color.WHITE}/{new_prefix}"
            ...
            for i, sn in enumerate(subnets[:display_limit]):
                if not is_v6 and sn.prefixlen < 31:
                    hs = list(sn.hosts())
                    detail = (
                        f"  {Color.DARK_GRAY}hosts: {Color.WHITE}{hs[0]} – {hs[-1]}" if hs else ""
                    )
                else:
                    detail = ""
                print(
                    f"{Color.DARK_GRAY}  [{Color.LIGHT_RED}{i + 1:>3}{Color.DARK_GRAY}]"
