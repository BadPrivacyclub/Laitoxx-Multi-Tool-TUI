...
def web_crawler():
    ...
    seed_url = input(
        f"{Color.DARK_GRAY}  - {Color.WHITE}Enter the seed URL to start crawling from: {Color.RESET}"
    ).strip()
    ...
    try:
        max_pages_str = (
            input(
                f"{Color.DARK_GRAY}  - {Color.WHITE}Enter max number of pages to crawl [20]: {Color.RESET}"
            ).strip()
            or "20"
        )
    ...
    save_to_file = (
        input(
            f"\n{Color.DARK_GRAY}[{Color.DARK_RED}⛧{Color.DARK_GRAY}]{Color.WHITE} Do you want to save the list of crawled pages? (y/n) [y]: {Color.RESET}"
        )
        .strip()
        .lower()
    )
