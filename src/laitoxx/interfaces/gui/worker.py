...
PLUGIN_COMMAND_TIMEOUT_SECONDS = 30
...
def _safe_plugin_path(plugin_path: str, relative_path: str) -> str:
    base = os.path.abspath(plugin_path or ".")
    full = os.path.abspath(os.path.join(base, relative_path))
    if os.path.commonpath([base, full]) != base:
        raise PermissionError(f"Path escapes plugin directory: {relative_path}")
    return full


def _sanitize_command_for_log(args: list[str], api_key: str = "") -> str:
    masked = ["********" if api_key and part == api_key else part for part in args]
...
class Worker(QObject):
    ...
    report_ready = pyqtSignal(str)  # emits generated HTML report path
    ...
    def _run_lua_plugin(self):
        try:
            ...
            leak_plugin = is_leakosint_plugin(self.lua_plugin_meta)

            def _graph_ready(path: str) -> None:
                graph_paths.append(path)
                if not leak_plugin:
                    self.graph_ready.emit(path)
            ...
            result = run_lua_plugin(
                ...
                graph_callback=_graph_ready,
            )
            if result:
                self._emit_update(result)
                if leak_plugin:
                    try:
                        report_path = create_leakosint_report(
                            result,
                            graph_paths=graph_paths,
                            query=str(self.input_data or ""),
                        )
                        if report_path:
                            self.report_ready.emit(report_path)
                    except Exception as report_error:
                        logging.error("LeakOSINT report generation failed", exc_info=True)
                        self.error.emit(f"LeakOSINT report error: {report_error}")
    ...
    def _run_with_writer(self, func):
        writer = SignalWriter(
            self.update.emit, transform=remove_ansi_codes, cancel_event=self._cancel_event
        )
    ...
    def _run_shell_step(self, step, action_type, action_value, current_input, plugin_path):
        ...
        api_key = step.get("api_key", "") if step.get("requires_api_key") else ""
        self.update.emit(
            f"Executing {action_type.replace('_', ' ')}: {_sanitize_command_for_log(args, api_key)}"
        )
        ...
        try:
            result = subprocess.run(
                args,
                shell=False,
                input=current_input if action_type in {"batch_script", "shell_script"} else None,
                capture_output=True,
                text=True,
                encoding="utf-8",
                errors="ignore",
            ...
            self.error.emit(
                f"Error: Step timed out after {PLUGIN_COMMAND_TIMEOUT_SECONDS} seconds."
            )
            return False, ""
    ...
    def _build_subprocess_args(self, step, action_type, action_value, current_input, plugin_path):
        if action_type == "command":
            args = self._parse_plugin_command(step, action_value, current_input)
            executable = shutil.which(args[0])
            if executable is None:
                raise ValueError(f"Command not found: {args[0]}")
            return [executable, *args[1:]]

        script_path = _safe_plugin_path(plugin_path, action_value)
        if not os.path.isfile(script_path):
            raise FileNotFoundError(
                f"{action_type.replace('_', ' ').title()} '{action_value}' not found."
            )
        if action_type == "batch_script":
            if platform.system() != "Windows":
                raise ValueError("Batch scripts are only supported on Windows.")
            return ["cmd.exe", "/d", "/c", script_path]
        if action_type == "shell_script":
            if platform.system() == "Windows":
                raise ValueError("Shell scripts are not supported on Windows.")
            return ["sh", script_path]
        raise ValueError(f"Unsupported shell step type: {action_type}")

    def _parse_plugin_command(self, step, action_value, current_input):
        args = shlex.split(action_value, posix=platform.system() != "Windows")
        if not args:
            raise ValueError("Empty command step.")
        command = args[0].lower()
        allowed = {str(item).lower() for item in step.get("allowed_commands", [])}
        allowed.update(ALLOWED_PLUGIN_COMMANDS)
        if command not in allowed:
            raise PermissionError(f"Command '{args[0]}' is not allowed for plugin execution.")
        api_key = step.get("api_key", "") if step.get("requires_api_key") else ""
        return [
            current_input if part == "{input}" else api_key if part == "{AKEY}" else part
            for part in args
        ]
    ...
    def _run_python_script_step(self, action_value, current_input, plugin_path):
        ...
        try:
            ...
            self.error.emit(
                f"Error: Script timed out after {PLUGIN_COMMAND_TIMEOUT_SECONDS} seconds."
            )
            return False, ""
        if result.stdout.strip():
            self.update.emit(f"STDOUT:\n{result.stdout.strip()}")
