...
class MainWindow(QMainWindow):
    ...
    def _build_sidebar(self):
        ...
        for btn in (
            self.btn_settings,
            self.btn_load_theme,
            self.btn_create_theme,
            self.btn_plugin_builder,
            self.btn_graph_editor,
    ...
    def _set_background(self, path):
        ...
        try:
            ...
            if os.path.commonpath([bg_dir, background_path]) != bg_dir:
    ...
    def _start_worker_thread(
        ...
        *,
        ...
        worker.report_ready.connect(self._on_leak_report_ready)
        if connect_graph:
    ...
    def _on_leak_report_ready(self, report_path: str) -> None:
        self._append_output(f"\n[Report] LeakOSINT HTML report saved: {report_path}\n")
        reply = QMessageBox.question(
            self,
            translator.get("leak_report_title"),
            translator.get("leak_report_open_prompt", path=report_path),
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.Yes,
        )
        if reply == QMessageBox.StandardButton.Yes:
