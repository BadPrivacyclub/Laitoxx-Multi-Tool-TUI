...
class ImageSearchWindow(QDialog):
    ...
    def _build_context_panel(self) -> QWidget:
        ...
        for panel in (self._ctx_search, self._ctx_editor, self._ctx_hashes, self._ctx_forensics):
    ...
    def _load_file(self, path: str) -> None:
        ...
        if not os.path.isfile(path):
            QMessageBox.warning(
                self, _t("error", "Error"), f"{_t('file_not_found', 'File not found')}:\n{path}"
            )
            return
        try:
            pil = Image.open(path)
    ...
    def _start_search(self) -> None:
        if not self._pil_original:
            QMessageBox.information(
                self, _t("error", "Error"), _t("is_no_image", "Please load an image first.")
            )
            return
        if not HAS_REQUESTS:
            QMessageBox.warning(
                self,
                _t("error", "Error"),
                _t("is_no_requests", "requests library not installed.\nSearch unavailable."),
            )
            return
    ...
    def _compare_images(self) -> None:
        ...
        path, _ = QFileDialog.getOpenFileName(
            ...
            "",
            _t("is_compare_file_filter", "Images (*.png *.jpg *.jpeg *.webp *.bmp);;All Files (*)"),
        )
        if not path:
            return
        try:
    ...
    def _start_forensics(self) -> None:
        ...
        if not any(checks.values()):
            QMessageBox.information(
                self, _t("error", "Error"), _t("is_no_checks", "Select at least one check.")
            )
            return
