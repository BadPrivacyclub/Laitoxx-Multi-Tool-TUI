...
class LuaCodeEditor(QPlainTextEdit):
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if event.key() in (Qt.Key.Key_Return, Qt.Key.Key_Enter):
            ...
            if stripped.endswith(("then", "do", "else", "function", "repeat")) or re.search(
                r"function\s*\(.*\)\s*$", stripped
            ):
                indent += "    "
...
def check_lua_syntax(source: str) -> list[dict]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if (
            re.search(r"\bif\b", code_no_strings)
            and not re.search(r"\bthen\b", code_no_strings)
        ...
        # Empty function body
        if (
            re.match(r"\s*function\b.*\bend\b\s*$", code_no_strings)
            and "return" not in code_no_strings
            and "print" not in code_no_strings
        ):
            issues.append(
                {
                    "line": i,
        ...
        if (
            "host." in code_no_strings
            and "host:" not in code_no_strings
            and re.search(r"host\.\w+\s*\(", code_no_strings)
        ):
            issues.append(
                {
                    "line": i,
    ...
    if (
        "return plugin" not in source
        and "return plugin\n" not in source
        and source.strip()
        and "local plugin" in source
    ):
        issues.append(
            {
                "line": len(lines),
...
def generate_plugin_template(
    plugin_name: str, plugin_type: str, author: str, description: str, target_os: list[str]
) -> str:
...
class PluginBuilderWindow(QDialog):
    ...
    def _save_plugin(self):
        ...
        # Determine save path
        lua_dir = str(ROOT_DIR / "lua_plugins")
