...
class StyleEditor(QWidget):
    ...
    def __init__(self, parent=None, is_edge: bool = False):
        ...
        def _lbl(t):
            label = QLabel(t)
            label.setStyleSheet(f"color: {_TEXT_SEC}; font-size: 11px;")
            return label
...
class _PropPanel(GlassPanel):
    ...
    def __init__(self, title: str, fields: list[str], parent=None):
        ...
        self._title_lbl.setStyleSheet(
            f"color: {_ACCENT}; font-size: 12px; font-weight: 700; letter-spacing: 0.5px;"
        )
...
_NODE_TYPE_ACTIONS: dict[str, list[tuple[str, str]]] = {
    ...
    "Email": [("ge_act_check_email", "Validate Email"), ("ge_act_gmail_osint", "Gmail Osint")],
    "IP": [("ge_act_check_ip", "Check IP")],
    "Website": [
        ("ge_act_info_website", "Info Website"),
...
class GraphEditorWindow(QDialog):
    ...
    def __init__(
        self, parent=None, theme_data: dict | None = None, lua_plugins: list | None = None
    ):
        super().__init__(parent)
            ...
            if "color" in ss:
                ...
                if m:
                    ...
                    if cur in {v.lower() for v in _ACCENT_VALS}:
                        ...
                        changed = True
                    elif cur in {v.lower() for v in _SEC_VALS} or cur in {
                        v.lower() for v in _DIM_VALS
                    }:
    ...
    def _save_graph_as(self):
        path, _ = QFileDialog.getSaveFileName(
            self, _t("ge_save_as"), f"{self._graph.name}.json", "Graph JSON (*.json);;All Files (*)"
        )
        if path:
            self._do_save(path)
    ...
    def _edit_edge_by_id(self, edge_id: str):
        ...
        if not edge:
            return
        dlg = EdgeDialog(
            self, edge=edge, nodes=self._graph.nodes, title=_t("ge_dialog_edit_edge", id=edge.id)
        )
        if dlg.exec():
            dlg.get_edge()
            self._refresh_all()
