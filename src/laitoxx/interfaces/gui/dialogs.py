...
class ThemeEditorDialog(QDialog):
    ...
    def _open_color_picker(self):
        ...
        dialog.setWindowTitle(
            translator.get("edit_color_title", element=self.element_selector.currentText())
        )
        if dialog.exec():
...
class UserAgreementDialog(QDialog):
    def __init__(self, parent=None):
        ...
        try:
            with open(USER_AGREEMENT_FILE, encoding="utf-8") as f:
        ...
        buttons.addButton("I Disagree", QDialogButtonBox.ButtonRole.RejectRole).clicked.connect(
            self._on_disagree
        )
...
class LuaPluginConfigDialog(QDialog):
    ...
    def __init__(self, parent=None, plugin_meta=None):
        ...
        self.setWindowTitle(
            translator.get(
                "lua_plugin_settings_title", name=plugin_meta.name if plugin_meta else "Plugin"
            )
        )
        ...
        if plugin_meta and plugin_meta.config_schema:
            ...
            if hasattr(raw, "values"):
                ...
                try:
                    schema = [
                        _lua_config_entry(raw[i]) for i in range(1, 100) if raw[i] is not None
                    ]
