...
from laitoxx.core.settings.paths import LEGACY_LUA_SETTINGS, ROOT_DIR
...
LUA_PLUGIN_DIR = str(ROOT_DIR / "lua_plugins")
LUA_PLUGIN_CONFIG_FILE = LEGACY_LUA_SETTINGS
TOOL_VERSION = "2.2.2"
...
class HostAPI:
    ...
    def http_get(self, url, timeout=15, headers_table=None):
        ...
        try:
            ...
            resp = self._session.get(str(url), timeout=_bounded_timeout(timeout), headers=headers)
    ...
    def http_post(
        self, url, data=None, timeout=15, headers_table=None, content_type="application/json"
    ):
        ...
        try:
            ...
            resp = self._session.post(
                str(url), data=body, timeout=_bounded_timeout(timeout), headers=headers
            )
            resp.raise_for_status()
            return resp.text
        except Exception as e:
...
def _bounded_timeout(value, *, default: int = 15, minimum: int = 1, maximum: int = 30) -> int:
    try:
        timeout = int(value)
