...
from laitoxx.shared.graph.model import SHAPE_ID_TO_D3, SHAPE_ID_TO_TOKENS, Graph, Node
...
def generate_mermaid(graph: Graph) -> str:
    ...
    for link_idx, edge in enumerate(graph.edges):
...
def generate_html(graph: Graph, lang: str = None, theme: dict = None) -> str:
    ...
    graph_json = _json.dumps({"nodes": nodes_data, "edges": edges_data}, ensure_ascii=False)
